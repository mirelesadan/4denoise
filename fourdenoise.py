...
def get_surface_tilt_and_direction(surface, units='rad', show_results=True, figsize=(4, 2.5)):
    """
...
def apply_majority_filter(cluster_map, kernel_size=3, iterations=1):
    ...
    def majority_filter(pixel_values):
        # SciPy’s mode with axis=None returns a scalar ModeResult
        m = mode(pixel_values, axis=None)
        return m.mode[0]   # extract the mode value

        # — or, without SciPy —
        # vals, counts = np.unique(pixel_values, return_counts=True)
        # return vals[np.argmax(counts)]

    footprint = np.ones((kernel_size, kernel_size))
    filtered = ndimage.generic_filter(
        cluster_map, majority_filter,
        footprint=footprint, mode='constant', cval=0
    )
    for _ in range(iterations-1):
        filtered = ndimage.generic_filter(
            filtered, majority_filter,
            footprint=footprint, mode='constant', cval=0
        )
    return filtered
...
def reconstruct_height(xGrad, yGrad, y_bds_flat, x_bds_flat, iterations=10, threshold_percent=0.5, 
                       plot=True, returnGradients=True, max_window_size=23):
...
# def rotate_height(height, angle, axis='x'):
#     """
#     Rotate the height map by the specified angle about a chosen pivot so that the fitted
#     lines become horizontal. This function applies a proper rotation in the (axis, height)
#     plane.

#     Parameters:
#     - height: 2D numpy array representing the height map.
#     - angle: float, rotation angle in degrees.
#     - axis: str, 'x' for rotation in the (x, height) plane, 'y' for rotation in the (y, height) plane.

#     Returns:
#     - height_rot: 2D numpy array representing the rotated height map.
#     """
#     theta = np.deg2rad(angle)  # Convert angle to radians

#     if axis == 'x':
#         # For the x-axis rotation, treat the horizontal coordinate (x) and height (h)
#         # Choose pivot as (x0, h0) with x0 = 0 and h0 = median of the first column.
#         x0 = 0
#         h0 = np.median(height[:, 0])
#         x_coords = np.arange(height.shape[1])
#         height_rot = np.empty_like(height)
        
#         # For each column j, update the height value:
#         # h' = (x - x0)*sin(theta) + (h - h0)*cos(theta) + h0,
#         # where x is the column index.
#         for j in range(height.shape[1]):
#             height_rot[:, j] = x_coords[j] * np.sin(theta) + (height[:, j] - h0) * np.cos(theta) + h0

#     elif axis == 'y':
#         # For the y-axis rotation, treat the vertical coordinate (y) and height (h)
#         # Choose pivot as (y0, h0) with y0 = 0 and h0 = median of the first row.
#         y0 = 0
#         h0 = np.median(height[0, :])
#         y_coords = np.arange(height.shape[0])
#         height_rot = np.empty_like(height)
        
#         # For each row i, update the height value:
#         # h' = (y - y0)*sin(theta) + (h - h0)*cos(theta) + h0,
#         # where y is the row index.
#         for i in range(height.shape[0]):
#             height_rot[i, :] = y_coords[i] * np.sin(theta) + (height[i, :] - h0) * np.cos(theta) + h0
#     else:
...
def filter_and_split_cluster_map(cluster_map: np.ndarray, min_size: int = 10) -> np.ndarray:
    """
    Split each integer-labeled cluster into its connected-component "islands",
    reassign any island smaller than min_size to the majority neighbor, and
    renumber islands so labels run from 1..M consecutively.

    Parameters
    ----------
    cluster_map : np.ndarray[int], shape (H, W)
        Input 2D array of cluster labels (0 = background).
    min_size : int
    ...
    np.ndarray[int], shape (H, W)
        Filtered and fully-split cluster map, with new labels 1..M.
    """
    ...
    # Step 1: split each label into connected-component islands
    for lab in np.unique(cluster_map):
        if lab == 0:
            continue
        mask_lab = (cluster_map == lab)
        comp_lab, num_comp = label(mask_lab)
    ...
    # Step 2: reassign small islands based on majority neighbor
    for island_id, size in island_sizes.items():
        if size < min_size:
            ys, xs = np.nonzero(comp_global == island_id)
            for y, x in zip(ys, xs):
                y0, y1 = max(y-1, 0), min(y+2, H)
                x0, x1 = max(x-1, 0), min(x+2, W)
                neigh = comp_global[y0:y1, x0:x1].ravel()
                neigh = neigh[(neigh != island_id) & (neigh != 0)]
                if neigh.size:
                    comp_global[y, x] = np.bincount(neigh).argmax()
                else:
                    comp_global[y, x] = 0

    # Step 3: renumber remaining islands consecutively
    new_map = np.zeros_like(comp_global)
    new_ids = np.unique(comp_global)
    new_ids = new_ids[new_ids != 0]
    for new_label, old_id in enumerate(new_ids, start=1):
        new_map[comp_global == old_id] = new_label
...
class HyperData:
    ...
    def standardize(self, method='local'):
        ...
        method : str, optional
            ...
            - 'global': Standardize the dataset globally (entire dataset has zero mean and unit variance).
            - 'local': Standardize each individual image in the dataset (each CxD image has zero mean and unit variance).
            Default is 'global'.
    
        Returns
        -------
        standardized_tensor : HyperData
            The standardized dataset with zero mean and unit variance, either globally or locally.
        ...
        if method == 'global':
            ...
            standardized_tensor = (self.array - mean) / (std + 1)
    
        elif method == 'local':
            # Local standardization: Standardize each CxD image in the AxB grid
            A, B, C, D = self.array.shape
            # Compute mean and std for each CxD image (last two dimensions)
            mean = np.mean(self.array, axis=(2, 3), keepdims=True)
            std = np.std(self.array, axis=(2, 3), keepdims=True)
    ...
    def crop(self, ylim=None, xlim=None, 
                   kylim=None, kxlim=None):
        """
    ...
    #TODO Make function to get real space image
    
    def get_dp(self, y=None, x=None, real_mask=None, reciprocal_mask=None, **flat_kwargs):
        """
            ...
            if y in operations:
                if y == 'random':
                    if self.ndim == 4:
                        ...
                        idx = int(self.shape[0]*operations[y]())
                        ...
                        mean, flat_mask = mask_and_average(reduced_data.array, return_mask=True, function='sum_2d', threshold='upper', **flat_kwargs)
            ...
            if isinstance(y, tuple) and isinstance(x, tuple):
               ...
               return ReciprocalSpace(np.mean(self.array[ymin:ymax, :, :, :], axis=(0,1)))
                ...
                if self.ndim == 4: 
                    ...
                    raise ValueError("Picking diffraction pattern currently supported only for 3D or 4D arrays.")
               ...
               return ReciprocalSpace(np.mean(self.array[:, xmin:xmax, :, :], axis=(0,1)))
                ...
                raise ValueError("y and x must be either both tuples, both integers, one tuple-one integer, or one integer-one 'None'.")
    ...
    def get_centers(self, r, ref_coords, method='CoM'):
        """
        ...
        `ref_coords` may be:
          - a NumPy array of shape (n_peaks, 2), used for every DP, or
          - a list of lists (4D) [[ ... ]] of shape (Ny, Nx), where each entry
            is an array-like of (n_ij, 2) coordinates, or
          - a list (3D) of length B, where each entry is array-like (n_i, 2).
        ...
        if self.ndim == 4:
            ...
            if isinstance(ref_coords, list):
                ...
                for i in tqdm(range(Ny), desc="Computing centers (4D)"):
                    row = []
                    for j in range(Nx):
                        coords_ij = ref_coords[i][j]
                        dp = self.get_dp(i, j)
                        # pass through show=False
                        c = dp.get_centers(r=r,
                                          ref_coords=np.asarray(coords_ij),
                                          show=False,
                                          method=method)
                        row.append(c)
                    centers.append(row)
        ...
        # 3D case
            ...
            if isinstance(ref_coords, list):
                ...
                for i in tqdm(range(B), desc="Computing centers (3D)"):
                    coords_i = ref_coords[i]
                    dp = self.get_dp(i)
                    c = dp.get_centers(r=r,
                                       ref_coords=np.asarray(coords_i),
                                       show=False,
                                       method=method)
                    centers.append(c)
                return centers
    ...
    def get_intensities(self,
        ...
        centers : array-like or list, optional
            ...
            If list:
              - list of length Ny, each an inner list of length Nx of (n_ij,2) arrays
                for 4D, or
              - list of length B of (n_i,2) arrays for 3D.
        ref_coords, method, compute_resBg, residual_frac, **resBg_kwargs :
            Passed to each dp.get_intensities(...) call.
        ...
        if self.ndim == 4:
            ...
            if isinstance(centers, list):
                ...
                for i in tqdm(range(Ny), desc="Row"):
                    row_ints = []
                    for j in range(Nx):
                        dp_centers = centers[i][j]    # shape (n_ij, 2)
                        dp = self.get_dp(i, j)
        ...
        # 3D case
            ...
            if isinstance(centers, list):
                ...
                for i in tqdm(range(B), desc="DP"):
                    dp_centers = centers[i]   # shape (n_i, 2)
                    dp = self.get_dp(i)
    ...
    # def get_strains(self, centers, ref_centers=None, ang=0, g_vector=1,
    #                 r=None, r_inner=None, r_outer=None,):
    #     """        
    #     r_innter and r_outer are used for extracting the reference region if not provided
    #     """
                
    #     if ref_centers is None and centers is None:
    #         raise ValueError("Either 'ref_centers', 'centers', or none of them are allowed to be undefined.")
        
    #     elif ref_centers is None:
    #         # Automatically extract approximate flat region centers
    #         if r_inner or r_outer is None:
    #             r = (self.shape[-2] + self.shape[-1])/4
    #             reduced_data = self.apply_mask(r_inner=0.6*r, r_outer=0.8*r,)
    #         else: 
    #             reduced_data = self.apply_mask(r_inner, r_outer)
    #         mean, flat_mask = mask_and_average(reduced_data.array, return_mask=True, function='sum_2d', threshold='upper', percentile=5, show_mask=False)
    #         ref_centers = np.mean(centers[flat_mask], axis=0)

    #     elif centers is None:
    #         centers = self.get_centers(r=r, ref_coords=ref_centers, method='CoM')
    ...
    #         # Define g_vectors using conjuugate spots
    ...
    #         """ Strain arrays """
    #         ydim = centers.shape[0]
    #         xdim = centers.shape[1]
    ...
    #         for g_vector in range(3):
                
    #             g_vector = g_vector+1 # We want the indices to be [1,2,3]
                
    #             # Define g_vectors using conjugate spots
    #             g1 = centers[:, :, g_vector-1, :] - centers[:, :, g_vector+2, :]
    #             g2 = (centers[:, :, g_vector, :] + centers[:, :, g_vector+1, :]) / 2 - \
    #                  (centers[:, :, (g_vector+3) % 6, :] + centers[:, :, (g_vector+4) % 6, :]) / 2
    ...
    #                     # Uniaxial
    #                     eyy_maps[g_vector-1,i,j] = 1-U[0, 0]
    #                     exx_maps[g_vector-1,i,j] = 1-U[1, 1]
            
    #                     # Shear
    #                     exy_maps[g_vector-1,i,j] = U[1, 0]
            
    #                     # Rotation
    #                     erot_maps[g_vector-1,i,j] = np.arccos(R[0, 1])
            
    #         # Get the average strain map in each independent component
    ...
    def get_clusters(self, n_PCAcomponents, n_clusters, r_centerBeam, std_Threshold=0.2, power=1,
                 ...
                 outer_ring=None, polar=False):
        """
        Perform PCA-based dimensionality reduction and clustering on a 4D dataset, returning a cluster map
        while offering various clustering methods and visualization options.
        ...
        std_Threshold : float, optional
            Threshold for standard deviation filtering as a fraction of the maximum standard deviation.
            Pixels below this threshold are masked out. Default is 0.2.
        power : int, optional
            Exponent to apply to the dataset for emphasis (e.g., power=2 squares the values). Default is 1.
        clustering_method : str, optional
            Clustering method to use. Options include "k-means" and "hierarchical". Default is "k-means".
        plotStdMask : bool, optional
            Whether to display a mask of the high standard deviation regions. Default is True.
        plotScree : bool, optional
            Whether to display a scree plot of PCA variance explained. Default is True.
        plotClusterMap : bool, optional
            Whether to display a 2D color-coded cluster map. Default is True.
        plot3dClusterMap : bool, optional
            Whether to display a 3D scatter plot of the clustered data in PCA space. Default is False.
        filter_size : int, optional
            Size of the majority filter to smooth the cluster map. Default is 3.
        cluster_cmap : str, optional
            Name of the colormap to use for visualizing clusters. Default is None (uses 'gnuplot').
        filter_iterations : int, optional
            Number of iterations to apply the majority filter. Default is 1.
        outer_ring : int, optional
            Radius of the outer boundary for masking. Default is None.
        polar : bool, optional
            Whether to apply polar coordinate-based masking. Default is False.
    
        Returns
        -------
        cluster_map : numpy.ndarray
            A 2D array of shape `(A, B)` representing the cluster labels for each pixel in the original
            dataset.
        ...
        Notes
        -----
        - The dataset is first masked to remove the center beam and optionally an outer ring (if `polar=True`).
        - Pixels with low standard deviation (below `std_Threshold`) are further masked out.
        - PCA is performed on the masked dataset to reduce its dimensionality to `n_PCAcomponents`.
        - Clustering is performed using the selected `clustering_method`.
        - The clustering results are visualized as a 2D cluster map and optionally a 3D scatter plot.
        ...
        if plot3dClusterMap:
            ...
            for i in range(n_clusters):
                color = colormap(i)
                ax.scatter(data_reduced[clusters == i, 0], data_reduced[clusters == i, 1],
                           data_reduced[clusters == i, 2], c=[color], label=f'Cluster {i + 1}', s=2, alpha=0.5)
            plt.show()
